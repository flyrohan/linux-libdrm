...
static rgbpixel_t getpixel_32bpp(bmp_t *bmp, int x, int y)
{
	...
	rgb.b = ((*pdata & mask[3]) >> bmp->boffset_blue)
			<< (8 - bmp->bsize_blue);
	rgb.g = ((*pdata & mask[2]) >> bmp->boffset_green)
			<< (8 - bmp->bsize_green);
	rgb.r = ((*pdata & mask[1]) >> bmp->boffset_red)
			<< (8 - bmp->bsize_red);
	rgb.a = ((*pdata & mask[0]) >> bmp->boffset_alpha)
			<< (8 - bmp->bsize_alpha);
 ...
 * bmp_bitperpixel:
 */
int bmp_bitperpixel(bmp_t *bmp)
{
	return (int)(!bmp ? 0 : bmp->bmpinfo_h.bit_count);
}
...
/**
 * bmp_load:
 * - bmpfile : file name to open
 * - memory  : loaded memory address
 */
bmp_t *bmp_load(const char *bmpfile, void *memory)
{
	...
	case 32:
		...
		if (bmpi->compression == BI_RGB) {
			...
			mask = (unsigned *)malloc(sizeof(unsigned)*4);
			mask[3] = 0x000000FF; /* blue mask */
			mask[2] = 0x0000FF00; /* green mask */
			mask[1] = 0x00FF0000; /* red mask */
			mask[0] = 0xFF000000; /* alpha mask */
			bmp->palette = (struct palette_s *)mask;
			bmp->boffset_blue = 0;
			bmp->boffset_green = 8;
			bmp->boffset_red = 16;
			bmp->boffset_alpha = 24;
			bmp->bsize_blue = 8;
			bmp->bsize_green = 8;
			bmp->bsize_red = 8;
			bmp->bsize_alpha = 8;
